...
def ms_bfs_based_rpq(
    ...
    common_alphabet = (
        dfa_matrix.boolean_decomposition.keys()
        & nfa_matrix.boolean_decomposition.keys()
    )

    current_front = create_initial_front(
        dfa_state_count, nfa_state_count, start_state_pairs
    )
    ...
    for dfa_final_state in dfa_matrix.final_states:
        for index, nfa_start in enumerate(nfa_matrix.start_states):
            state_block = explored_states[
                dfa_state_count * index : dfa_state_count * (index + 1)
            ]
